...
import NroMap from '../../components/ui/NroMap';
...
export default function NroLookup() {
 ...
 return <div className="tool-page">
  <header className="tool-page-header"><h1>NRO Lookup and Map</h1><p>Search official organization names and aliases locally, then explore geographic context.</p><SourceNote ids={['nro']} /></header>
  ...
  <PrintMeta />
  <p className="print-only">Name/alias filter: {query || 'None'}. Country filter: {country === 'all' ? 'All countries' : country}. This printout reflects the current view, including possible name candidates.</p>
  <div className="lookup-controls screen-only">
  ...
  <NroMap organizations={results.map(r => r.organization)} />
