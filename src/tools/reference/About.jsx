...
export default function About() {
  return <div className="tool-page">
    ...
    <section className="review-card"><h2>Privacy and device storage</h2>
      <p>NRO name and technology category searches, the bundled map overview and comparisons using coordinates entered locally work without external services. There are no analytics, advertising or remote fonts. The hosting provider can receive connection information and retain access logs.</p>
      <p>The map has two separate, optional external services, both off by default. Enabling Wikipedia place search sends the public institution or campus query you explicitly submit, plus connection information, to Wikimedia. Enabling OpenStreetMap street detail sends connection information, this site’s origin and the map areas you view to OpenStreetMap. Street tiles receive no institution names or worksheet answers. Their privacy policies are linked beside each control.</p>
      <p>Map queries, selected locations and external-service choices stay in memory and are cleared on reload. Turning a service off stops new requests and discards pending search results; it cannot recall requests already sent. Location sources can be approximate, incomplete or wrong: confirm the intended campus. Geographic proximity does not establish a relationship or risk.</p>
    ...
    <section className="review-card"><h2>Verification and limitations</h2>
      ...
      <p><a href="./THIRD_PARTY_NOTICES.txt">Third-party software, font and map data notices</a> · <a href="./LICENSE.txt">Software license</a>. The local map uses public-domain Natural Earth boundaries and individually cited Wikidata/Wikipedia location records. Location sources are separate from the official NRO list and do not imply government verification of a pin. Linked policies and third-party marks retain their own terms.</p>
